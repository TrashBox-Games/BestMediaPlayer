...
import { View, Text, StyleSheet, Button, ScrollView, TextInput, Switch } from 'react-native';
...
export default function HttpServerScreen2(): JSX.Element {
  ...
  useEffect(() => {
    // Get the device's IP address and load assets
  ...
  // Reload assets from the assets directory
  const reloadAssets = async (): Promise<void> => {
    try {
      addLog('Reloading assets...');
      await loadAssets();
      addLog('Assets reloaded successfully');
    } catch (error) {
      addLog(`Error reloading assets: ${(error as Error).message}`);
    }
  };

  return (
    <View style={styles.container}>
      <Text style={styles.title}>HTTP Server</Text>
      ...
      {isRunning && (
        <View style={styles.serverInfo}>
          ...
          <Text style={styles.serverUrl}>http://{ipAddress}:{port}</Text>
          <Text>Open this URL in a web browser to view the page</Text>
        </View>
      )}
